...
/**
 * Solver for Knapsack Problem using dynamic programming - memoization.
 */
public class KnapsackSolverMemoization {
   ...
   */
  public KnapsackSolverMemoization(List<Item> allItems, int capacity) {
  ...
  public int solveMaxPrice() {
    return this.solveHelper(this.capacity, this.allItems.size());
  }

  /**
   * Gets the result that should be at the cell with given weight num and item num in the cache.
   * Also modifies the cache to store previous subproblems' solutions, so they do not need to be
   * solved again.
   *
  ...
  private int solveHelper(int curWeightNum, int curItemNum) {
    ...
    if (curItemNum == 0) {
      this.cache[curWeightNum][curItemNum] = 0;
      return this.cache[curWeightNum][curItemNum];
    }
    ...
    if (item.getWeight() <= curWeightNum) {
      this.cache[curWeightNum][curItemNum] = Math.max(item.getPrice() + solveHelper(curWeightNum - item.getWeight(),
    ...
    // If the item is too heavy, it just uses the subproblem of weight and previous items answer.
      ...
      this.cache[curWeightNum][curItemNum] = solveHelper(curWeightNum, curItemNum - 1);
      return this.cache[curWeightNum][curItemNum];
    }
  }
